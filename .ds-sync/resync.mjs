...
// failure (grading is the agent's job). A failed stage stops the chain;
...
// run: a build that dies before package-build's OUT wipe (malformed config,
// [OUT_UNSAFE]) leaves the PREVIOUS run's artifacts on disk, and reading
// them would dress a failed run in the prior run's anchor and partitions.
...
if (aborted) {
  ...
  let refDrift = false;
  {
    ...
    if (pool.length) {
      ...
      const single = (n) => cfgObj.overrides?.[n]?.cardMode === 'single' || capOf(n)?.portal === true;
      const picks = new Set();
  ...
  // capture preserves the signal for the retry.
  ...
  if (sbCur && stages.capture.ok !== false && (!refDrift || canaryPicks.length)) {
    try {
...
const ok = Object.values(stages).every((s) => (s.skipped === null ? s.ok : s.skipped !== 'prior_failure'));
const verdict = {
  version: 2,
  ok,
  shape,
