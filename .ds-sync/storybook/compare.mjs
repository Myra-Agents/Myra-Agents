...
import { KEY_RECIPE, gradeKeyFrom, renderHashFor, sbBaseShaFor, userPreviewFor } from '../lib/sync-hashes.mjs';
  ...
  if (c.sourceKey && prevCapture && prevCapture.gradeKey !== gradeKey && (prevCapture.keyRecipe ?? 0) !== KEY_RECIPE) {
    ...
    if (prevCapture.gradeKey === oldGradeKeyFor(c)) {
      ...
      adopt();
        ...
        !userPreviewFor(c.name)) {
      // Artifacts churned but story sources provably didn't (and no user
      // preview source — owned or hand-modified cache — which srcSha can't
      // vouch for) — adopt; sampled below. A
